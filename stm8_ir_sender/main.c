...
int main(void){
	...
	/* устанавливается частота работы таймера. второй параметр это период таймера.
		 то есть частота генерируемого сигнала. */
	//52 - 1 при DIV_8 это ~ 38 khz. 16Mhz / 8 = 2000Khz. 2000Khz / 52 ~ 38Khz
	//52 это 0.0264ms
	//0.56 это примерно 21-н проход(0.0264 * 21 ~ 0.56ms)
	...
	//второй полупериод шим сигнала(по продолжительности такой же как и второй)
	...
	/* включение генерации прерывания для 2-го(CC2) канала таймера.
	 	 прерывание будет вызываться каждый полупериод то есть каждые 19Khz ! */
...
/* массив задержек активности(старшие 4 бита) и молчания(младшие 4-ре бита)
...
void tim2_pre_start_init(void){
	pc = 0;
	delays_index = 0;
	/* при старте передача выключена и тут устанавливаем значения заглушек
	 	 чтобы сразу ее включить. */
	lim1 = 1;
	lim2 = 1;
	last_imp = 0;
}
...
INTERRUPT_HANDLER(TIM2_CAP_COM_IRQHandler, 14){
	if(TIM2->SR1 & TIM2_SR1_CC2IF){ //2-й канал таймера
		//GPIOD->ODR ^= GPIO_PIN_5; //для отладки можно дергать эту ножку и смотреть на осцилограф
		pc++;
		if(pc == lim1){
		...
		if(pc == lim2){ //весь период для IR бита отработали. переходим к следующему.
			delays_index++;
			//если еще есть биты задержек
			if(delays_index < DELAYS_COUNT){
				...
				lim2 *= DOT_56;
				lim2 += lim1;
			}else{ //все биты задержек былы обработаны
				last_imp = 1; //осталось дать последний импульс
				lim1 = DOT_56;
			}
			pc = 0;
